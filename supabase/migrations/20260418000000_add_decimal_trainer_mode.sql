alter table public.user_preferences
  add column if not exists trainer_practice_mode text not null default 'POSITIVE',
  add column if not exists trainer_left_decimal_digits smallint not null default 2,
  add column if not exists trainer_right_decimal_digits smallint not null default 2;

alter table public.user_preferences
  drop constraint if exists user_preferences_trainer_practice_mode_check;

alter table public.user_preferences
  add constraint user_preferences_trainer_practice_mode_check
  check (trainer_practice_mode in ('POSITIVE', 'DECIMAL'));

alter table public.user_preferences
  drop constraint if exists user_preferences_trainer_left_decimal_digits_check;

alter table public.user_preferences
  add constraint user_preferences_trainer_left_decimal_digits_check
  check (trainer_left_decimal_digits between 1 and 8);

alter table public.user_preferences
  drop constraint if exists user_preferences_trainer_right_decimal_digits_check;

alter table public.user_preferences
  add constraint user_preferences_trainer_right_decimal_digits_check
  check (trainer_right_decimal_digits between 1 and 8);

alter table public.progress_logs
  add column if not exists practice_mode text not null default 'POSITIVE',
  add column if not exists left_decimal_digits smallint not null default 0,
...
alter table public.progress_logs
  drop constraint if exists progress_logs_practice_mode_check;

alter table public.progress_logs
  add constraint progress_logs_practice_mode_check
  check (practice_mode in ('POSITIVE', 'DECIMAL'));

alter table public.progress_logs
  drop constraint if exists progress_logs_left_decimal_digits_check;

alter table public.progress_logs
  add constraint progress_logs_left_decimal_digits_check
  check (left_decimal_digits between 0 and 8);

alter table public.progress_logs
  drop constraint if exists progress_logs_right_decimal_digits_check;

alter table public.progress_logs
  add constraint progress_logs_right_decimal_digits_check
...
alter table public.progress_logs
  ...
  check (right_operand ~ '^[0-9]+(?:\.[0-9]+)?$');

alter table public.progress_logs
  drop constraint if exists progress_logs_correct_answer_check;

alter table public.progress_logs
  add constraint progress_logs_correct_answer_check
  check (correct_answer ~ '^-?[0-9]+(?:\.[0-9]+)?$');

alter table public.progress_logs
  drop constraint if exists progress_logs_submitted_answer_check;

alter table public.progress_logs
  add constraint progress_logs_submitted_answer_check
  check (submitted_answer ~ '^-?[0-9]+(?:\.[0-9]+)?$');
    ...
    select
      ...
      case
        ...
        when p.practice_mode = 'DECIMAL' then
          p.digits_left::text || 'd/' || p.left_decimal_digits::text || 'dp × ' ||
